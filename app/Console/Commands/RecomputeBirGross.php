...
class RecomputeBirGross extends Command
{
  
  protected $signature = 'bir-gross {brcode : Branch Code} {date : YYYY-MM-DD} {--dateTo=NULL : Date To} {--percentage=0 : Percentage} {--print=false : Print} {--final=false : Final}';
  ...
  */
  ...
  public function handle() {
    ...
    foreach ($this->monthInterval($date, $to) as $key => $d) {
      ...
      $ds['grschrg'] = number_format((($ds['grschrg']*($percent/100))), 2, '.', '');
      $ds['sale'] =  $ds['grschrg'] - ($ds['vat_ex'] + $ds['totdisc']);
      $ds['sale_chrg'] = $ds['sale'] - $ds['sale_cash'];
